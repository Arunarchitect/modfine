...
def activateEmail(request, user, to_email):
    ...
    if email.send():
        ...
        messages.error(request, f'Problem sending email, Check if you have typed it correctly')
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            for key, error in list(form.errors.items()):
                if key == 'captcha' and error[0] == 'This field is requires.':
...
def PasswordResetConfirm(request, uidb64, token):
    ...
    if user is not None and account_activation_token.check_token(user,token):
        if request.method == 'POST':
            ...
            if form.is_valid():
                form.save()
                messages.success(request, "your password has been set, You may go ahead and <b>login </b> now. ")  
                return redirect('home')
            else:
                for error in list(form.errors.values()):
                    messages.error(request, error)
